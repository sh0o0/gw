...
func newRenameCmd() *cobra.Command {
	return &cobra.Command{
		...
		Short: "Rename branch and move worktree directory",
		Args:  cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			oldBranch := args[0]
			newBranch := args[1]
			return renameBranchAndWorktree(oldBranch, newBranch)
		},
	}
}

func renameBranchAndWorktree(oldBranch, newBranch string) error {
	...
	current, _ := gitx.CurrentWorktreePath("")
	isCurrentWorktree := current == oldPath

	newPath, err := worktree.ComputeWorktreePath("", newBranch)
	if err != nil {
		return fmt.Errorf("failed to compute new worktree path: %w", err)
	}

	if _, err := os.Stat(newPath); err == nil {
		return fmt.Errorf("worktree directory already exists: %s", newPath)
	} else if !errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("failed to check target worktree directory: %w", err)
	}

	if _, err := gitx.Cmd("", "show-ref", "--verify", "--quiet", "refs/heads/"+newBranch); err == nil {
		return fmt.Errorf("branch already exists: %s", newBranch)
	}

	fmt.Fprintf(os.Stderr, "Renaming branch [%s] to [%s]\n", oldBranch, newBranch)
	fmt.Fprintf(os.Stderr, "Moving worktree: %s -> %s\n", oldPath, newPath)

	if _, err := gitx.Cmd(oldPath, "branch", "-m", oldBranch, newBranch); err != nil {
		return fmt.Errorf("failed to rename branch: %w", err)
	}

	if err := fsutil.EnsureDir(filepath.Dir(newPath)); err != nil {
		// Attempt to revert branch rename before returning
		gitx.Cmd(oldPath, "branch", "-m", newBranch, oldBranch)
		return fmt.Errorf("failed to create directory: %w", err)
	}

	if _, err := gitx.Cmd("", "worktree", "move", oldPath, newPath); err != nil {
		gitx.Cmd(oldPath, "branch", "-m", newBranch, oldBranch)
		return fmt.Errorf("failed to move worktree: %w", err)
	}

	fmt.Fprintf(os.Stderr, "✓ Successfully renamed branch and moved worktree\n")

	if isCurrentWorktree {
		rel, _ := relativePathFromGitRoot()
		navigateToRelativePath(newPath, rel)
	}
